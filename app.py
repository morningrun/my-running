...
# 이미지 base64 변환 함수 (아이콘 및 마스코트 공용)
...
# mascot.png를 읽어와서 스마트폰 홈 화면 아이콘 및 내부 마스코트로 활용
...
# 2. 화면 스타일 및 스마트폰 홈 화면 앱 메타 태그 적용 CSS
st.markdown(f"""
    <head>
        <title>200CREW</title>
...
if not df.empty:
    ...
    # 마스코트 이미지 출력 세팅
    if icon_base64:
        mascot_html = f'<img src="data:image/png;base64,{icon_base64}" style="width: 56px; height: 56px; border-radius: 50%; border: 2px solid #38BDF8; object-fit: contain; background-color: #FFFFFF; padding: 3px;">'
    else:
